...
Some models might be restricted and need authenticated."):
        ...
        if command == 'load_in_bits':
            def _check_valid(command: str) -> bool:
                if command in ('4', '8', '16', '32'):
                    return True
                return False

            if not _change_single(command, "'load_in_bits' is for 'quantization' method. \
If the VALUE is 16, then model is load in 16 bits (2 bytes) per parameters.\
Choose from: '4', '8', '16', '32'.", _check_valid):
                break
            continue
...
def _self_destruction(parser_arg: argparse.Namespace):
    ...
    if parser_arg.pypi:
        utils_ext.self_destruction('pypi')
    elif parser_arg.github:
        utils_ext.self_destruction('github')
